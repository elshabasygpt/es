...
import { FileSignature, Scale, Globe, AlertTriangle, RefreshCw, CheckCircle } from "lucide-react";
...
export const TermsClient = () => {
    ...
    const icons = [
        <CheckCircle key="check" className="w-10 h-10 text-green-500" />,
        <Scale key="scale" className="w-10 h-10 text-green-500" />,
        <Globe key="globe" className="w-10 h-10 text-green-500" />,
        <AlertTriangle key="alert" className="w-10 h-10 text-amber-500" />,
        <RefreshCw key="refresh" className="w-10 h-10 text-blue-500" />,
    ];
                ...
                <Container className="relative z-10 text-center max-w-4xl">
                    <ScrollReveal>
                        <div className="inline-flex items-center justify-center w-28 h-28 rounded-[2rem] bg-white/10 border border-white/20 mb-8 backdrop-blur-sm shadow-xl">
                            <FileSignature className="w-14 h-14 text-green-400" />
                        </div>
                                ...
                                {p.sections.map((section: any, index: number) => (
                                    <div key={index} className="scroll-mt-24" id={`section-${index}`}>
                                        <div className="flex items-center gap-6 mb-4">
                                            <div className={`w-20 h-20 rounded-[1.25rem] flex items-center justify-center shrink-0 border shadow-sm ${
                                                index === 3 ? "bg-amber-50 border-amber-100" : 
                                                index === 4 ? "bg-blue-50 border-blue-100" : 
